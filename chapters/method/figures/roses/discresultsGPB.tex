...
\draw[thin, draw=blue!50!white] circle[radius={.5+1.8*\aovDataGPBSIFTMRFmean}];
\draw[thin, draw=blue!50!white] circle[radius={.5+1.8*\aovDataGPBSIFTMRFmax}];
\draw[thin, draw=blue] ({\sectorSize/2},\aovDataGPBSIFTMRF[0])
	\foreach \x [count=\n] in \aovDataGPBSIFTMRF
	{ -- ({(\n-1)*\sectorSize+(\sectorSize/2)}:{1.8*\x+.5}) };

% Draw the Fasle Positive
\foreach \x/\a [count=\n] in {1/345,3/15,6/15,12/15}
{
	\draw[very thin, draw=red!50!white] (0,0) circle [radius={\fpDataGPBSIFTMRFlogScaleLabelsPos[\x]}];
	\pgfmathsetmacro\fplabel{{\fpDataGPBSIFTMRFlogScaleLabels[\x]}}
\node at (\a+90:{\fpDataGPBSIFTMRFlogScaleLabelsPos[\x]}) {\fplabel};
\node at ({\a-90}:{\fpDataGPBSIFTMRFlogScaleLabelsPos[\x]}) {\fplabel};
}	

	
\draw[very thin, draw=red] ({\sectorSize/2},\fpDataGPBSIFTMRFlogScale[0])
	\foreach \x [count=\n] in \fpDataGPBSIFTMRFlogScale
